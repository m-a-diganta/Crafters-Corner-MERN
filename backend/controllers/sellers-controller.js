...
const getSellers = async (req, res, next) => {
  ...
  res.json({ sellers: (await sellers).map(seller => seller.toObject({ getters: true })) });
};
...
const signupSeller = async (req, res, next) => {
  ...
  const createdSeller = new Seller({
    name,
    email,
    image: 'https://i.pinimg.com/564x/7d/34/d9/7d34d9d53640af5cfd2614c57dfa7f13.jpg',
    password,
    productList: [],
    courseList: []
  });
  ...
  res.status(201).json({ seller: createdSeller.toObject({ getters: true }) });
};
