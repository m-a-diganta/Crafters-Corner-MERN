...
const getProductById = async (req, res, next) => {
  ...
  if (!product) {
    const error = new HttpError('Could not find a product for such product ID', 404);
    return next(error);
  }
...
const getProductsBySellerId = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError('Could not find products for the given seller', 500);
    return next(error);
  }
  ...
  res.json({products: (await products).map(place=> place.toObject({getters: true}))});
};
...
const getProductsByCategory = async (req, res, next) => {
  ...
  if (!products || products.length === 0) {
    return next(new HttpError('Could not find products for such category', 404));
  }
...
const updateProduct = async (req, res, next) => {
  ...
  try {
    ...
    if (err.name === 'ValidationError') {
      ...
      const error = new HttpError(`Product validation failed: ${errors.join(', ')}`, 422);
      return next(error);
    } else {
      // Handle other errors
