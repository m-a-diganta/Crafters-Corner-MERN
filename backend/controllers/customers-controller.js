...
const getCustomers = async (req, res, next) => {
  ...
  res.json({ customers: (await customers).map(customer => customer.toObject({ getters: true })) });
};
...
const deleteCustomer = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError('Something went wrong, could not delete customer.', 500);
    return next(error);
  }
